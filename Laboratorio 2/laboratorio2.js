...
let images1 = cuadros.map(
  (cuadro) => new Image(cuadro.title, cuadro.artist, cuadro.date)
);

// Paso 5: Crear un nuevo arreglo images2 usando la función de fábrica getImage
let images2 = images1.map((image) =>
  getImage(image.title, image.artist, image.date)
);

// Mostrar el contenido de images2 en la consola
images2.forEach((image) => {
  console.log(
    `Título: ${image.title}, Artista: ${image.artist}, Año: ${image.date}`
  );
});
